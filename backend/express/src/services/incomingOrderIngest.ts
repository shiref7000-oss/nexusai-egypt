...
export async function ingestIncomingOrder(input: {
  ...
  try {
    ...
    if (created) {
      try {
        const { enqueueCodConfirmation } = await import('./whatsapp/codFlow');
        await enqueueCodConfirmation({
          userId: input.userId,
          orderId: order.id as string,
          customerPhone: o.customerPhone,
          customerName: o.customerName,
        ...
        logger.warn('COD WhatsApp enqueue skipped', { userId: input.userId, error: waMsg });
      }
    }
