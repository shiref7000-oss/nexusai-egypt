...
export const openaiAdapter: ProviderAdapter = {
  ...
  complete: (req, startAttempt) => completeOpenAI(req, startAttempt),
};

async function completeOpenAI(req: CompletionRequest, startAttempt: number): Promise<CompletionResult> {
  const key = env.OPENAI_API_KEY;
  if (!key) {
    return fail('openai', 'OPENAI_API_KEY not configured', 0);
  }
  ...
  for (let i = startAttempt; i < MAX_ATTEMPTS; i++) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return fail('openai', lastError, latencyMs);
      }
  ...
  return fail('openai', lastError, 0);
}

function fail(provider: 'openai', error: string, latencyMs: number): CompletionResult {
  return {
    success: false,
    text: '',
    provider,
