...
import { startContextIntelligenceWorker } from './businessContext/workers';
...
/** Business Context Intelligence — crawl, embed, match (never on HTTP hot path). */
export const contextIntelligenceQueue = new Queue('context-intelligence', {
  connection: redis,
  defaultJobOptions: {
    ...defaultJobOptions,
    attempts: 2,
...
// ============================================================
// Workers (lazy — API sets RUN_WORKERS=false, nexusai-worker PM2 app runs them)
// ============================================================
type WorkerBundle = {
  aiWorker: Worker;
  workflowWorker: Worker;
  webhookOutboundWorker: Worker;
  contextIntelligenceWorker: Worker;
  whatsappOutboundWorker: Worker;
  notificationWorker: Worker;
};

let workerBundle: WorkerBundle | null = null;

export function startQueueWorkers(): WorkerBundle {
...
// ============================================================
  ...
  const contextIntelligenceWorker = startContextIntelligenceWorker();

  const { processWhatsAppOutboundJob } = require('./whatsapp/sendService');
  const whatsappOutboundWorker = new Worker(
    'whatsapp-outbound',
    async (job: Job) => processWhatsAppOutboundJob(job.data),
    {
      connection: redis,
      concurrency: 4,
  ...
  // Event handlers — monitoring & dead letter
...
webhookOutboundWorker.on('failed', async (job, err) => {
  ...
  whatsappOutboundWorker.on('failed', (job, err) => {
    if (job && job.attemptsMade >= (job.opts?.attempts || 4)) {
      handleFailedJob(job, err, 'whatsapp-outbound');
    }
    logger.error(`WhatsApp job ${job?.id} failed`, { error: err.message, userId: job?.data?.userId });
  });
...
// ============================================================
export async function addContextIntelligenceJob(data: {
  type: string;
  userId: number;
  sourceId?: number;
  storeUrl?: string;
  ...
  return contextIntelligenceQueue.add(`bci-${data.type}-${data.userId}-${Date.now()}`, data, {
    jobId: `bci-${data.type}-${data.userId}`,
    removeOnComplete: { age: 3600 * 48, count: 200 },
  });
}
...
export async function closeQueues() {
  ...
  if (workerBundle) {
    await Promise.all([
      workerBundle.aiWorker.close(),
      workerBundle.workflowWorker.close(),
      workerBundle.notificationWorker.close(),
      workerBundle.webhookOutboundWorker.close(),
      workerBundle.contextIntelligenceWorker.close(),
      workerBundle.whatsappOutboundWorker.close(),
    ]);
    workerBundle = null;
  }
  await Promise.all([
    ...
    contextIntelligenceQueue.close(),
    (await import('./whatsapp/queue')).whatsappOutboundQueue.close(),
