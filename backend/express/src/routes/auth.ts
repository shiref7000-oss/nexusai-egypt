...
import { getUserUsage, syncUserPlanLimit, type UsageSnapshot } from '../services/usage';
import {
  createPgUser,
  findPgUserByEmail,
  pgUserToAuthProfile,
...
import { resolveWorkspaceUserId } from '../utils/workspaceContext';
...
router.get('/me', authenticate, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const workspace = await resolveWorkspaceUserId(req);
    res.json({
      success: true,
      data: {
        user: {
          ...req.user,
          pgUserId: pgUserId ?? req.user?.pgUserId,
          ...(usage ? {
            monthlyRequestLimit: usage.monthlyLimit,
            monthlyRequestsUsed: usage.monthlyUsed,
            remaining: usage.remaining,
...
router.get('/workspace-context', authenticate, async (req: AuthenticatedRequest, res) => {
  const workspace = await resolveWorkspaceUserId(req);
  res.status(workspace.userId ? 200 : workspace.status || 400).json({
    success: Boolean(workspace.userId),
