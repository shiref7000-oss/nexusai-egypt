...
router.get('/orders/:id', [param('id').isUUID()], async (req: AuthenticatedRequest, res: Response) => {
  ...
  const order = await getIntegrationOrderDetail(uid, req.params.id);
  if (!order) return res.status(404).json({ success: false, error: 'Order not found' });

  const [statusHistory, incomingLogs] = await Promise.all([
    listOrderStatusHistory(uid, req.params.id),
    listIncomingWebhookLogs(uid, { orderId: req.params.id, limit: 20 }),
  ]);
...
router.patch('/orders/:id/status', [
  ...
  try {
    ...
    const existing = await getIntegrationOrder(uid, req.params.id);
    if (!existing) {
...
router.post('/logs/:logId/retry', async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!uid) return res.status(400).json({ success: false, error: 'User not linked to database' });
  try {
    const result = await retryWebhookLog(req.params.logId, uid);
...
router.post('/events/:eventId/replay', async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!uid) return res.status(400).json({ success: false, error: 'User not linked to database' });
  try {
    const result = await replayEvent(req.params.eventId, uid);
    res.json({ success: true, data: result });
  } catch (err: any) {
    res.status(404).json({ success: false, error: err.message });
  }
...
router.get('/:id', async (req: AuthenticatedRequest, res: Response) => {
  const uid = await pgUserId(req);
  if (!uid) return res.status(400).json({ success: false, error: 'User not linked to database' });
  const row = await getIntegration(uid, parseInt(req.params.id, 10));
...
router.post('/:id/regenerate-incoming-secret', async (req: AuthenticatedRequest, res: Response) => {
  const uid = await pgUserId(req);
  if (!uid) return res.status(400).json({ success: false, error: 'User not linked to database' });
  const row = await regenerateIncomingSecret(uid, parseInt(req.params.id, 10));
  if (!row) return res.status(404).json({ success: false, error: 'Not found' });
  res.json({
    success: true,
    data: {
...
router.patch(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    const uid = await pgUserId(req);
    if (!uid) return res.status(400).json({ success: false, error: 'User not linked to database' });
    const row = await updateIntegration(uid, parseInt(req.params.id, 10), req.body);
    if (!row) return res.status(404).json({ success: false, error: 'Not found' });
    res.json({ success: true, data: formatIntegrationRow(row) });
  }
);
...
router.get('/:id/webhooks', async (req: AuthenticatedRequest, res: Response) => {
  const uid = await pgUserId(req);
  if (!uid) return res.status(400).json({ success: false, error: 'User not linked to database' });
  const rows = await listWebhooks(uid, parseInt(req.params.id, 10));
...
router.patch('/webhooks/:webhookId', async (req: AuthenticatedRequest, res: Response) => {
  const uid = await pgUserId(req);
  if (!uid) return res.status(400).json({ success: false, error: 'User not linked to database' });
  const row = await updateWebhook(uid, req.params.webhookId, req.body);
  if (!row) return res.status(404).json({ success: false, error: 'Not found' });
...
router.get('/:id/api-keys', async (req: AuthenticatedRequest, res: Response) => {
  const uid = await pgUserId(req);
  if (!uid) return res.status(400).json({ success: false, error: 'User not linked to database' });
  const rows = await listApiKeys(uid, parseInt(req.params.id, 10));
  res.json({ success: true, data: rows });
});
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    const uid = await pgUserId(req);
    if (!uid) return res.status(400).json({ success: false, error: 'User not linked to database' });
    const key = await createApiKey(uid, parseInt(req.params.id, 10), req.body.name, req.body.permissions);
    res.status(201).json({
      success: true,
      data: key,
